...
class TestALUtest extends FreeSpec with ChiselScalatestTester {
    "ALU Test" in {
    test (new ALU1)//.withAnnotations( Seq(VerilatorBackendAnnotation))
     {c =>
// ALU operations
    val array_op = Array ( ALU_ADD , ALU_SUB , ALU_AND , ALU_OR , ALU_XOR ,
    ALU_SLL , ALU_SRL , ALU_COPY_A , ALU_COPY_B , ALU_XXX )
